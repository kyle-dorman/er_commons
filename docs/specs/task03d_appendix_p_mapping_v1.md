...
- A later source-general rule also applies to validated `full_page_numeric`
  tables that have no surviving Docling table pointer. Exactly one such table
  may own a native body-text item only when every valid provenance region is on
  that table's page and fully contained by its bbox. Coverage split across
  tables, partial or outside geometry, mixed or rejected provenance, furniture,
  and `document_index` descendants remain text. The traversal retains an exact
  text-pointer-to-producer-table ownership map so suppression is diagnosable.
  Each accepted decision also retains the physical page, table bbox, all text
  regions, and stable reason
  `single_full_page_table_contains_all_text_regions`.
...
| Block | Docling `#/texts/<n>` | Preserve label as `block_type`, `content_layer`, `orig` as both raw and initially canonical text, declared lossless normalization, all valid regions, and raw pointer. The producer's separately normalized `text` remains in raw lineage. Attach to the matching synthetic root. Captions are ordinary caption blocks linked from their table or figure. Validated full-page table ownership suppresses only the duplicate canonical block view; it never changes native text. | One text pointer may emit once. Each emitted block receives text and geometry mappings to the raw Docling asset. Invalid provenance is accounted for in the summary. Geometry-owned text remains recoverable through raw Docling evidence, carries its exact producer-table owner during traversal, and receives the durable downstream bridge disposition `canonical_table_geometry_owned_text` when hierarchy-relevant. |
...
| Manifest, inventory, summary, completion | materialized JSONL and assets | Manifest names canonical files in contract order. Inventory checksums every candidate-owned file. `observations/table_text_ownership.jsonl` publishes one versioned diagnostic per accepted geometry owner with text pointer, producer table ID, physical page, table bbox, text regions, and stable reason. The summary records its count, path, reason counts, reason-specific table-owned text accounting, existing total traversal accounting, invalid provenance, suppressed picture descendants, and validation results. Document structure separately persists `canonical_table_geometry_owned_text` for hierarchy-relevant suppressed text. Completion marks non-release candidate status and is written last. | Publish through staging with atomic no-clobber rename. Reuse requires exact identity, inventory, checksum, completion, and managed-file-set verification. The diagnostic sidecar is candidate evidence, not a canonical asset record. Raw Docling and clean-table artifacts are immutable evidence and are never rewritten by ownership projection. |
