...
def test_explicit_layout_route_requires_declared_regions(tmp_path) -> None:
    """Route validation fails before invoking a parser or touching a source PDF."""
    rendered = page.RenderedPage(10.0, 10.0, "", Image.new("RGB", (10, 10)))

    with pytest.raises(ValueError, match="requires at least one table region"):
        page.route_page_candidates(
            pdf_path=tmp_path / "not-opened.pdf",
            page_number=1,
            rendered=rendered,
            ruled_regions=[],
            detection={"complex_page_minimum_regions": 2},
