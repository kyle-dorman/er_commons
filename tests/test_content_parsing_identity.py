...
def test_tracked_config_selects_the_maintained_human_owned_producer() -> None:
    ...
    v2, _ = load_content_parsing_config(
    ...
    with pytest.raises(ValueError, match="contained relative"):
        ContentParsingConfig.model_validate(payload)
...
def test_hierarchy_producer_config_freezes_the_machine_policy() -> None:
    ...
    candidate, _ = load_content_parsing_config(
    ...
    with pytest.raises(ValueError, match="hierarchy policy"):
        ContentParsingConfig.model_validate(invalid)
