...
from er_commons.document_records.record_mapping import (
...
def test_document_and_child_record_id_shapes_are_distinct() -> None:
    ...
    with pytest.raises(MappingContractError, match="do not accept a local key"):
        make_record_id(
            extraction_id,
            "document",
            "deir_fixture",
    ...
    with pytest.raises(MappingContractError, match="require a local key"):
...
def test_record_id_collisions_are_rejected() -> None:
    ...
    with pytest.raises(MappingContractError, match="duplicate record IDs"):
...
def test_record_collections_keep_canonical_sequence_order() -> None:
    ...
    with pytest.raises(MappingContractError, match="tables sequence"):
...
def test_regions_must_stay_within_page_bounds() -> None:
    ...
    with pytest.raises(MappingContractError, match="out-of-bounds region"):
...
def test_partial_document_cannot_finalize_table_families() -> None:
    ...
    with pytest.raises(MappingContractError, match="partial document"):
...
def test_source_edition_override_must_propagate_exactly() -> None:
    ...
    with pytest.raises(MappingContractError, match="source-edition override"):
...
def test_referential_integrity_rejects_unknown_targets() -> None:
    ...
    with pytest.raises(MappingContractError, match="unknown references"):
...
def test_record_id_type_must_match_its_collection() -> None:
    ...
    with pytest.raises(MappingContractError, match="wrong-type record ID"):
...
def test_reference_type_must_match_the_relationship() -> None:
    ...
    with pytest.raises(MappingContractError, match="wrong-type reference"):
...
def test_table_and_family_membership_must_be_symmetric() -> None:
    ...
    with pytest.raises(MappingContractError, match="family membership differs"):
...
def test_identity_payload_is_content_bound_to_its_digest() -> None:
    ...
    with pytest.raises(MappingContractError, match="identity and manifest digest"):
...
def test_materialization_scope_selects_documents_from_full_release() -> None:
    ...
    with pytest.raises(MappingContractError, match="materialization scope differs"):
    ...
    with pytest.raises(MappingContractError, match="unknown release sources"):
    ...
    with pytest.raises(MappingContractError, match="differs from release source"):
...
def test_materialization_scope_binds_ordered_producer_runs() -> None:
    ...
    with pytest.raises(MappingContractError, match="producer run order"):
...
def test_canonical_text_changes_require_declared_normalization() -> None:
    ...
    with pytest.raises(MappingContractError, match="unexplained canonical text"):
...
def test_table_shape_must_match_its_cells() -> None:
    ...
    with pytest.raises(MappingContractError, match="cell count differs"):
...
def test_human_review_fields_are_rejected_at_any_depth() -> None:
    ...
    with pytest.raises(MappingContractError, match="human-review field"):
...
def test_section_hierarchy_cannot_contain_cycles() -> None:
    ...
    with pytest.raises(MappingContractError, match="hierarchy cycle"):
...
def test_cross_reference_status_must_match_its_targets() -> None:
    ...
    with pytest.raises(MappingContractError, match="exactly one target"):
...
def test_raw_link_stage_must_match_its_asset_producer() -> None:
    ...
    with pytest.raises(MappingContractError, match="producer differs"):
