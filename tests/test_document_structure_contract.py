...
import pytest
from document_structure_support import (
...
from er_commons.document_records.document_structure import (
    BridgeSourceEvidence,
    StructureContractError,
    normalize_alias,
)
from er_commons.document_records.document_structure import (
    validate_document_structure_contract as validate_contract,
)
...
def test_v2_schema_and_positive_fixture_are_valid() -> None:
    ...
    validate_document_structure_contract(BUNDLE)
...
def test_alias_records_and_ambiguous_targets_use_deterministic_order() -> None:
    ...
    with pytest.raises(StructureContractError, match="deterministic order"):
        validate_document_structure_contract(reversed_records)

    reversed_targets = copy.deepcopy(BUNDLE)
    alias_named(reversed_targets, "repeated heading")["targets"].reverse()
    with pytest.raises(StructureContractError, match="targets are not in document order"):
        validate_document_structure_contract(reversed_targets)
...
def test_ordered_children_are_exact_mixed_content_inverse() -> None:
    ...
    with pytest.raises(StructureContractError, match="ordered section children"):
        validate_document_structure_contract(invalid)
...
def test_section_sequences_and_persisted_order_are_deterministic() -> None:
    ...
    with pytest.raises(StructureContractError, match="contiguous sequence"):
        validate_document_structure_contract(wrong_sequence)
    ...
    with pytest.raises(StructureContractError, match="document order"):
        validate_document_structure_contract(wrong_order)
...
def test_cycles_and_nonincreasing_levels_fail() -> None:
    ...
    with pytest.raises(StructureContractError, match="greater than parent"):
        validate_document_structure_contract(invalid)
    ...
    with pytest.raises(StructureContractError):
        validate_document_structure_contract(cycle)
...
def test_semantic_sections_must_descend_from_the_body_root() -> None:
    ...
    with pytest.raises(StructureContractError, match="descend from the body root"):
        validate_document_structure_contract(invalid)
...
def test_heading_ownership_and_pre_root_toc_furniture_placement_are_strict() -> None:
    ...
    with pytest.raises(StructureContractError, match="heading block"):
        validate_document_structure_contract(wrong_heading)
    ...
    with pytest.raises(StructureContractError, match="pre-root"):
        validate_document_structure_contract(wrong_pre_root)
    ...
    with pytest.raises(StructureContractError, match="TOC content"):
        validate_document_structure_contract(wrong_toc)
    ...
    with pytest.raises(StructureContractError, match="furniture"):
        validate_document_structure_contract(wrong_furniture)
    ...
    with pytest.raises(StructureContractError, match="heading ownership"):
        validate_document_structure_contract(unowned_heading)
...
def test_content_placement_and_document_scope_cannot_contradict_records() -> None:
    ...
    with pytest.raises(StructureContractError, match="furniture content has a body placement"):
        validate_document_structure_contract(wrong_furniture_placement)
    ...
    with pytest.raises(StructureContractError, match="section escaped document scope"):
        validate_document_structure_contract(wrong_section_document)
    ...
    with pytest.raises(StructureContractError, match="alias escaped document scope"):
        validate_document_structure_contract(wrong_alias_document)
...
def test_tables_and_figures_inherit_sections_but_never_own_headings() -> None:
    ...
    with pytest.raises(StructureContractError, match="cannot own semantic headings"):
        validate_document_structure_contract(invalid)
...
def test_page_label_outcomes_cover_pages_without_text_items() -> None:
    ...
    with pytest.raises(StructureContractError, match="complete physical-page sequence"):
        validate_document_structure_contract(invalid)
...
def test_page_coverage_is_bound_to_the_accepted_control() -> None:
    ...
    with pytest.raises(StructureContractError, match="accepted control"):
        validate_document_structure_contract(truncated)


def test_page_label_state_value_evidence_and_conflict_are_coherent() -> None:
    invalid = copy.deepcopy(BUNDLE)
    page_label(invalid, 3)["resolved_label"] = "3"
    with pytest.raises(StructureContractError, match="resolution differs.*physical page 3"):
        validate_document_structure_contract(invalid)

    invalid = copy.deepcopy(BUNDLE)
    page_label(invalid, 4)["visible_label"]["value"] = "3"
    with pytest.raises(StructureContractError, match="resolution differs.*physical page 4"):
        validate_document_structure_contract(invalid)

    missing_evidence = copy.deepcopy(BUNDLE)
    page_label(missing_evidence, 2)["visible_label"]["evidence_refs"] = []
    with pytest.raises(StructureContractError, match="no source anchors"):
        validate_document_structure_contract(missing_evidence)
...
def test_page_label_precedence_cannot_be_relabelled(
    ...
    with pytest.raises(StructureContractError, match=f"physical page {physical_page}"):
        validate_document_structure_contract(invalid)
...
def test_conflicting_sources_cannot_be_published_as_resolved() -> None:
    ...
    with pytest.raises(StructureContractError, match="physical page 4"):
        validate_document_structure_contract(invalid)
...
def test_alias_normalization_collision_and_toc_target_policy() -> None:
    ...
    with pytest.raises(StructureContractError, match="normalization"):
        validate_document_structure_contract(invalid)

    invalid = copy.deepcopy(BUNDLE)
    alias_named(invalid, "repeated heading")["resolution_status"] = "unique"
    with pytest.raises(StructureContractError, match="collision state"):
        validate_document_structure_contract(invalid)
    ...
    with pytest.raises(StructureContractError, match="TOC alias"):
        validate_document_structure_contract(invalid)
    ...
    with pytest.raises(StructureContractError, match="kind and target type"):
        validate_document_structure_contract(wrong_target_type)
...
def test_printed_page_aliases_require_an_existing_resolved_label() -> None:
    ...
    validate_document_structure_contract(valid)

    unresolved = copy.deepcopy(BUNDLE)
    add_printed_page_alias(unresolved, physical_page=3, raw_label="3")
    with pytest.raises(StructureContractError, match="does not exist with declared type"):
        validate_document_structure_contract(unresolved)
    ...
    with pytest.raises(StructureContractError, match="does not exist with declared type"):
        validate_document_structure_contract(nonexistent)
    ...
    with pytest.raises(StructureContractError, match="invalid provenance"):
        validate_document_structure_contract(wrong_provenance)


def test_bridge_requires_coverage_unique_keys_targets_and_exact_producers() -> None:
    missing = copy.deepcopy(BUNDLE)
    missing["bridge_entries"].pop()
    with pytest.raises(StructureContractError, match="cover"):
        validate_document_structure_contract(missing)
    ...
    with pytest.raises(StructureContractError, match="keys collide"):
        validate_document_structure_contract(duplicate_key)
    ...
    with pytest.raises(StructureContractError, match="stable key differs"):
        validate_document_structure_contract(duplicate_target)
    ...
    with pytest.raises(StructureContractError, match="producer differs"):
        validate_document_structure_contract(changed_producer)
    ...
    with pytest.raises(StructureContractError, match="producer correspondence differs"):
...
def test_bridge_permitted_replacement_dispositions_are_closed() -> None:
    ...
    validate_document_structure_contract(bridge, evidence)

    entry["disposition"] = "other"
    with pytest.raises(StructureContractError, match="not permitted"):
        ...
        validate_document_structure_contract(bridge, changed_evidence)

    unbacked = copy.deepcopy(BUNDLE)
    unbacked["bridge_entries"].append(copy.deepcopy(entry))
    with pytest.raises(StructureContractError, match="source-evidence coverage differs"):
        validate_document_structure_contract(unbacked)
...
def test_mapped_bridge_targets_must_be_canonical_blocks() -> None:
    ...
    with pytest.raises(StructureContractError, match="not a canonical block"):
        validate_document_structure_contract(invalid)
    ...
    with pytest.raises(StructureContractError, match="stable key differs"):
        validate_document_structure_contract(swapped)
...
def test_control_and_correspondence_fail_closed() -> None:
    ...
    with pytest.raises(StructureContractError, match="semantic_file_set_sha256"):
        validate_document_structure_contract(control)

    correspondence = copy.deepcopy(BUNDLE)
    correspondence["correspondence_report"]["undeclared_difference_count"] = 1
    with pytest.raises(StructureContractError, match="undeclared differences"):
        validate_document_structure_contract(correspondence)
