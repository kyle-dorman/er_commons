"""Cross-record policy tests for hierarchy inference v1."""
...
import pytest
from hierarchy_inference_support import (
    VALID_BUNDLE,
...
def test_valid_bundle_passes_every_cross_record_policy() -> None:
    ...
    validate_hierarchy_inference_bundle(VALID_BUNDLE)
...
def test_missing_r08_eligibility_fails_closed() -> None:
    ...
    with pytest.raises(HierarchyInferenceContractError, match="R08 eligibility absent"):
...
def test_picture_owned_non_caption_text_must_select_r01() -> None:
    ...
    with pytest.raises(HierarchyInferenceContractError, match="R01 eligibility differs"):
        validate_hierarchy_inference_bundle(bundle)
...
def test_picture_owned_caption_cannot_be_promoted_by_an_anchor() -> None:
    ...
    with pytest.raises(HierarchyInferenceContractError, match="picture caption rule differs"):
        validate_hierarchy_inference_bundle(bundle)
...
def test_strict_toc_match_cannot_claim_canonical_basis() -> None:
    ...
    with pytest.raises(HierarchyInferenceContractError, match="canonical TOC match differs"):
        validate_hierarchy_inference_bundle(bundle)
...
def test_strict_toc_match_cannot_carry_native_pdf_evidence() -> None:
    ...
    with pytest.raises(HierarchyInferenceContractError, match="non-native TOC match"):
        validate_hierarchy_inference_bundle(bundle)
...
def test_named_semantic_mutation_fails(
    ...
    with pytest.raises(HierarchyInferenceContractError):
        validate_hierarchy_inference_bundle(invalid_bundle)
...
def test_hierarchy_edges_must_remain_in_reading_order() -> None:
    ...
    with pytest.raises(HierarchyInferenceContractError, match="edges differ"):
        validate_hierarchy_inference_bundle(bundle)
...
def test_hierarchy_roots_must_remain_in_reading_order() -> None:
    ...
    with pytest.raises(HierarchyInferenceContractError, match="roots differ"):
        validate_hierarchy_inference_bundle(bundle)
...
def test_sparse_root_is_valid_only_with_exact_review_warning() -> None:
    ...
    with pytest.raises(HierarchyInferenceContractError, match="sparse hierarchy warnings differ"):
        validate_hierarchy_inference_bundle(bundle)
