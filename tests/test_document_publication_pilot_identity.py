...
def test_checked_in_identity_is_the_exact_three_source_pilot_recipe() -> None:
    """Legacy identity stays readable without claiming current-code compatibility."""
    ...
    assert validated.value == record["extraction_id"]
    owned = record["preimage"]["corpus_workflow_contract"]["owned_code"]
    assert any(item["path"].startswith("src/er_commons/corpus_extraction/") for item in owned)
...
def test_preflight_selects_run_spec_owners_in_manifest_order(tmp_path: Path) -> None:
    ...
    source_ids, evidence = build_production_scope_evidence(
        spec=spec,
        identity=identity,
        data_root=tmp_path,
    )
...
def test_preflight_rejects_pilot_owner_order_that_differs_from_manifest(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="sealed manifest order"):
        build_production_scope_evidence(spec=spec, identity=identity, data_root=tmp_path)


def _run_spec(manifest_relative: Path, source_ids: list[str]) -> DocumentRunSpec:
    configs = {
        "content_parsing": "configs/baseline.json",
        "heading_evidence_parsing": "configs/hierarchy.json",
        "record_mapping": "configs/record_mapping.json",
        "hierarchy_inference": "configs/correction.json",
        "document_structure": "configs/document_structure.json",
        "document_reference_linking": "configs/document_reference_linking.json",
    }
    return DocumentRunSpec.model_validate(
        {
