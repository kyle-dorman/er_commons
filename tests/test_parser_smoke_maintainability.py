...
RUNTIME_ROOT = Path("src/er_commons/parser_smoke")
...
def test_public_workflow_remains_a_short_application_shell() -> None:
    ...
    assert {
        "er_commons.parser_smoke.publication",
        "er_commons.parser_smoke.reporting",
        "er_commons.parser_smoke.source_processing",
    } <= imported_modules
...
def test_behavior_tests_do_not_depend_on_private_workflow_helpers() -> None:
    ...
    source = Path("tests/test_parser_smoke.py").read_text()
    tree = ast.parse(source)
    workflow_imports = [
        node
        for node in tree.body
        if isinstance(node, ast.ImportFrom) and node.module == "er_commons.parser_smoke.workflow"
    ]
