...
def test_review_manifest_is_disposable_and_checksummed(tmp_path: Path) -> None:
    ...
    path = write_review_manifest(
        tmp_path,
        request=ReviewRequest("exv1-test", "source", (1,), ("page", "table")),
        recipe=RenderRecipe(
            renderer="review-render",
            renderer_version="1.2.3",
            arguments=("--page", "1"),
            inputs=(ReviewInput("source_pdf", "sources/source.pdf", "a" * 64),),
        ),
        generated_files=(GeneratedReviewFile(output, 1, "page"),),
    )
