...
from er_commons.document_records.document_structure import StructureContractError
...
from er_commons.document_records.document_structure.support import _bridge_payload
...
def test_bridge_is_built_from_complete_independent_correspondence() -> None:
    ...
    with pytest.raises(StructureContractError, match="coverage differs"):
        build_cross_producer_bridge(
            items,
            hierarchy_producer_run_id="prv1-" + "3" * 64,
            baseline_producer_run_id="prv1-" + "4" * 64,
...
def test_producer_text_alignment_rejects_ambiguous_bbox_drift() -> None:
    ...
    with pytest.raises(DocumentStructureInvariantError, match="align uniquely"):
...
def test_sections_project_sparse_hierarchy_toc_furniture_and_nontext() -> None:
    ...
    sections, placed = build_document_sections(
        content,
