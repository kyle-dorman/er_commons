...
from er_commons.document_publication.records import DOCUMENT_PROCESS_NAMES
from er_commons.document_publication.retained_evidence import RetainedEvidenceError
from er_commons.document_publication.workflow import (
...
def test_success_publishes_complete_candidate_then_exactly_reuses(tmp_path: Path) -> None:
    ...
    def executor(request: DocumentExecutionRequest) -> ProcessOutcome:
        nonlocal calls
        calls += 1
        assert request.source_id == "alpha"
        assert request.attempt_root.name.startswith("txv1-")
        assert request.resources.outer_process_deadline_seconds == 20
        return ProcessOutcome(_result(tmp_path, "alpha"), False, 0, "")

    completion = publish_document(data_root, spec_path, "alpha", executor=executor)
    reused = publish_document(data_root, spec_path, "alpha", executor=executor)
...
def test_reuse_is_invalidated_by_current_run_spec_controls(tmp_path: Path) -> None:
    ...
    first = publish_document(data_root, spec_path, "alpha", executor=executor)
    payload = json.loads(spec_path.read_text())
    payload["resource_policy"]["queue_capacity"] = 9
    spec_path.write_text(json.dumps(payload))
    second = publish_document(data_root, spec_path, "alpha", executor=executor)
...
def test_interrupted_recovery_reports_corrupt_event_path(tmp_path: Path) -> None:
    """Corrupt retained state cannot escape as a pathless decoder exception."""
    data_root, spec_path = _workspace(tmp_path, retry_limit=1)
    with pytest.raises(KeyboardInterrupt):
        publish_document(
            data_root,
    ...
    corrupt_path = attempt_root / "state_events/0002.json"
    corrupt_path.write_text("{")

    with pytest.raises(RetainedEvidenceError) as captured:
        publish_document(data_root, spec_path, "alpha", executor=lambda *args: pytest.fail())

    assert captured.value.path == corrupt_path
    assert captured.value.subject == "state event"


def test_reuse_reports_mismatched_attempt_record_path(tmp_path: Path) -> None:
    """A published candidate cannot silently ignore contradictory attempt evidence."""
    data_root, spec_path = _workspace(tmp_path)
    publish_document(
        data_root,
        spec_path,
    ...
    attempt_root = next((data_root / "pipelines/test/task_03f/attempts").iterdir())
    record_path = attempt_root / "attempt_record.json"
    record = json.loads(record_path.read_text())
    record["disposition"] = "failed_terminal"
    write_json_atomic(record_path, record)

    with pytest.raises(RetainedEvidenceError) as captured:
        publish_document(data_root, spec_path, "alpha", executor=lambda *args: pytest.fail())

    assert captured.value.path == record_path
    assert captured.value.subject == "published attempt record"
...
def test_child_launch_failure_is_retained_with_observability(tmp_path: Path) -> None:
    ...
    with pytest.raises(OSError, match="worker launch failed"):
        publish_document(data_root, spec_path, "alpha", executor=fail_launch)
...
def test_extra_file_invalidates_reuse_instead_of_silent_acceptance(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="managed-file closure"):
        publish_document(data_root, spec_path, "alpha", executor=executor)
...
def test_publication_rejects_partial_stage_completion_handoff(tmp_path: Path) -> None:
    ...
    partial = result.model_copy(
        update={
            "stage_completions": {"linked_document": result.stage_completions["linked_document"]}
        }
    )

    with pytest.raises(ValueError, match="exact six product completions"):
        publish_document(
            data_root,
            spec_path,
            "alpha",
...
def test_publication_binds_final_root_to_cross_reference_completion(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="linked-document completion"):
        publish_document(
            data_root,
            spec_path,
            "alpha",
...
def test_pipeline_result_requires_all_stage_timings(tmp_path: Path) -> None:
    ...
    payload["stage_timings"] = {DOCUMENT_PROCESS_NAMES[0]: 0.1}
