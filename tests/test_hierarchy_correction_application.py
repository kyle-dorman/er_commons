...
from er_commons.hierarchy_correction.candidate_publication import CandidateWorkspace
from er_commons.hierarchy_correction.failures import RunStage
from er_commons.hierarchy_correction.publication_authorization import (
    VerifiedMachinePublication,
)
from er_commons.hierarchy_correction.single_build import SingleBuildResult
...
def test_existing_candidate_uses_verified_reuse_without_build(
    ...
    monkeypatch.setattr(application, "_reuse_candidate", lambda value: completion)
    monkeypatch.setattr(
        application,
        "build_single_semantic_candidate",
        lambda _inputs: pytest.fail("reuse must not build"),
    )

    assert application.run_hierarchy_correction(tmp_path, tmp_path / "config.json") == completion
...
def test_new_candidate_builds_exactly_once_then_authorizes_and_publishes(
    ...
    application.run_hierarchy_correction(tmp_path, tmp_path / "config.json")
