...
def test_group_cycle_is_fatal() -> None:
    ...
    with pytest.raises(MappingContractError, match="group cycle"):
...
def test_unknown_pointer_is_fatal() -> None:
    ...
    with pytest.raises(MappingContractError, match="unknown Docling pointer"):
        traverse_docling_document(document, {})
...
def test_duplicate_semantic_traversal_is_fatal() -> None:
    ...
    with pytest.raises(MappingContractError, match="duplicate Docling semantic traversal"):
        traverse_docling_document(document, {})
