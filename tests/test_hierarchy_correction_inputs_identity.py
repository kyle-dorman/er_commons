...
import er_commons.hierarchy_correction.inputs as input_module
...
from er_commons.hierarchy_correction.candidate_identity import (
...
from er_commons.source_freeze import SourceManifest, sha256_file


def _config(**updates: object) -> HierarchyCorrectionConfig:
    ...
    return HierarchyCorrectionConfig.model_validate(payload)
...
def _completion(config: HierarchyCorrectionConfig, manifest_sha256: str) -> dict[str, Any]:
    return {
        "schema_version": "1.0.0",
...
def test_loader_verifies_run_and_source_before_loading_semantic_inputs(
    ...
    inputs = load_hierarchy_correction_inputs(tmp_path, config)
