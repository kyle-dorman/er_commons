...
def test_corrupt_acquisition_record_names_artifact_and_nested_key(tmp_path: Path) -> None:
    """Integrity failures identify both the acquisition artifact and malformed field."""
    path = tmp_path / "acquisition_record.json"
    manifest = SourceManifest(
        manifest_schema_version="test.manifest.v1",
        source_release_version="release_v1",
        generated_at_utc="2026-07-24T00:00:00+00:00",
    ...
    with pytest.raises(ValueError, match=r"acquisition_record\.json: aggregates must be an object"):
        validate_acquisition_record(path, "a" * 64, manifest)

    path.write_bytes(b'{"schema_version":')
    with pytest.raises(ValueError, match=r"invalid JSON artifact .*acquisition_record\.json:1"):
        validate_acquisition_record(path, "a" * 64, manifest)


def test_corrupt_completion_record_names_missing_nested_key(tmp_path: Path) -> None:
    """A malformed sealed-file entry fails before unsafe indexing or file access."""
    path = tmp_path / "completion_record.json"
    manifest = SourceManifest(
        manifest_schema_version="test.manifest.v1",
        source_release_version="release_v1",
        generated_at_utc="2026-07-24T00:00:00+00:00",
        ...
        json.dumps(
            {
                ...
                "source_release_version": "release_v1",
                "source_spec_sha256": "a" * 64,
                "manifest": {"local_path": "manifest.json", "byte_size": 2},
                "acquisition_record": {},
                "visible_terms_note": {},
    ...
    with pytest.raises(
        ValueError,
        match=r"completion_record\.json: missing required key manifest\.sha256",
    ):
        verify_completion_record(tmp_path, path, "a" * 64, manifest)


def test_corrupt_landing_inventory_names_nested_shape(tmp_path: Path) -> None:
    """Inventory shape failures identify the exact page field before regeneration."""
    path = tmp_path / "landing_page_inventory.json"
    spec = release_spec([source_entry()])
    path.write_text(
        json.dumps(
            {
                "schema_version": "er_commons.landing_page_inventory.v1",
                "source_release_version": spec.release_id,
                "pages": [{"key": "draft", "url": spec.landing_pages[0].url, "links": {}}],
            }
        )
    )
    ...
    with pytest.raises(
        ValueError,
        match=r"landing_page_inventory\.json: pages\[0\]\.links must be a list",
    ):
        validate_landing_page_inventory(path, spec, tmp_path)
