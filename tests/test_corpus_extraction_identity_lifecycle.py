...
from er_commons.corpus_extraction.identity import (
    build_candidate_id,
    build_scope_id,
    build_transaction_id,
)
from er_commons.corpus_extraction.lifecycle import EventWriter
...
def test_state_and_completion_records_validate_against_accepted_v1_schema(
    ...
    completion_path = run_document(data_root, spec_path, "alpha", executor=executor)
    ...
    for definition, value in (
        ...
        ("document_resource_enforcement", resource_enforcement),
        ("document_observability", observability),
        ("document_completion", completion),
    ):
        Draft202012Validator({"$ref": f"#/$defs/{definition}", "$defs": schema["$defs"]}).validate(
            value
        )
