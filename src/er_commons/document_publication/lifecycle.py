...
from er_commons.document_publication.records import StateEvent
...
def read_events(root: Path) -> list[StateEvent]:
    """Read an attempt's typed, contiguous state-event history."""
    from er_commons.document_publication.retained_evidence import read_state_events

    return read_state_events(root)
