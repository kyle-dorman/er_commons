...
from er_commons.document_publication.identity import build_scope_id, canonical_digest
from er_commons.document_publication.lineage_preflight import ExecutionPreflight
...
from er_commons.document_publication.sources import resolve_manifest_source
...
class ProductionScopeSpec(Protocol):
    """Fields needed to join a current or historical readiness spec to a release."""

    @property
    def source_release_version(self) -> str: ...

    @property
    def source_manifest_relative_path(self) -> Path: ...

    @property
    def scope_kind(
        self,
    ) -> Literal["fixture", "engineering_smoke", "representative_pilot", "production_full"]: ...

    @property
    def document_processes(self) -> tuple[DocumentProcessSelection, ...]: ...
...
def build_production_scope_evidence(
    *, spec: ProductionScopeSpec, identity: dict[str, object], data_root: Path
    ...
    manifest_ref = scope.get("source_manifest")
    completion_ref = scope.get("release_completion")
