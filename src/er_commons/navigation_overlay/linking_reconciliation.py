...
class LinkingEntryOutcome(StrEnum):
...
def load_linking_document_index(document_root: Path) -> LinkingDocumentIndex:
    ...
    *,
    ...
    if marker_kind not in {"section", "table", "figure"}:
        return _decision_record(
            entry,
            LinkingEntryOutcome.UNSUPPORTED_ENTRY_SHAPE,
            (),
            (),
            (),
            False,
    ...
    if terminal_text is not None and not destination_ids:
        outcome = LinkingEntryOutcome.NO_DESTINATION_PAGE
    elif decision.outcome is ExactResolutionOutcome.NO_TEXT_MATCH:
        outcome = LinkingEntryOutcome.NO_TARGET_ALIAS
    elif decision.outcome is ExactResolutionOutcome.PARENT_SCOPE_MISMATCH:
        outcome = LinkingEntryOutcome.PARENT_SCOPE_MISMATCH
    elif decision.outcome is ExactResolutionOutcome.DESTINATION_PAGE_MISMATCH:
        outcome = LinkingEntryOutcome.DESTINATION_TARGET_PAGE_MISMATCH
    elif decision.outcome is ExactResolutionOutcome.AMBIGUOUS_TARGET:
        outcome = LinkingEntryOutcome.AMBIGUOUS_TARGET
    else:
        outcome = LinkingEntryOutcome.RESOLVED_UNIQUE
    return _decision_record(
        entry,
        outcome,
...
def reconcile_no_page_sections(
    *,
    entries_path: Path,
    ...
    indexes: dict[tuple[str, str], LinkingDocumentIndex] = {}
    ...
    for entry in iter_jsonl(entries_path):
        ...
        if key not in indexes:
            indexes[key] = load_linking_document_index(
                document_publications_root / "documents" / key[0] / key[1]
            )
        records.append(reconcile_linking_entry(entry, indexes[key], policy=policy))
