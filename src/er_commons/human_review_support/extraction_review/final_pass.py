...
from er_commons.human_review_support.extraction_review.final_pass_record import (
...
from er_commons.human_review_support.extraction_review.final_policy import (
    FINAL_PASS,
...
def prepare_final_extraction_review(
    data_root: Path,
    *,
    repo_root: Path,
    task_root_relative: str,
    task03i_root: Path,
    source_release_relative: str,
    scope_id: str,
    handoff_id: str,
    ...
    """Prepare source-free Gate A; the large per-source census remains explicit opt-in."""
    if scope_id != SCOPE_ID or handoff_id != HANDOFF_ID:
        raise ValueError("selected bindings differ from the historical final-pass profile")
    ...
    task03i = _load_task03i(task03i_root, repo_root)
    dependencies = _dependency_inventory(
        ...
        handoff,
        task03i,
        source_release_relative=source_release_relative,
    )
...
def publish_gate_a_preparation(record: dict[str, Any], output_root: Path) -> Path:
    """Publish the compact source-free Gate A record without clobbering bytes."""
    target = output_root / "records/gate_a_preparation.json"
...
def _implementation_inventory(repo_root: Path) -> dict[str, Any]:
    ...
    paths = [
        "src/er_commons/human_review_support/extraction_review/final_policy.py",
        "src/er_commons/human_review_support/extraction_review/final_pass.py",
        "src/er_commons/human_review_support/extraction_review/final_pass_record.py",
        "src/er_commons/human_review_support/extraction_review/toc_census.py",
        "src/er_commons/human_review_support/extraction_review/toc_census_support.py",
        "src/er_commons/human_review_support/extraction_review/toc_raw_scan.py",
        "src/er_commons/human_review_support/extraction_review/application.py",
        "src/er_commons/human_review_support/extraction_review/canonical_evidence.py",
        "src/er_commons/human_review_support/extraction_review/geometry.py",
        "src/er_commons/human_review_support/extraction_review/rendering.py",
