...
from er_commons.human_review_support.extraction_review.application import build_presentation
from er_commons.human_review_support.extraction_review.canonical_evidence import (
    page_review_evidence,
)
from er_commons.human_review_support.extraction_review.discovery import DiscoveredInputs
from er_commons.human_review_support.extraction_review.final_inputs import discover_final_inputs
...
from er_commons.human_review_support.extraction_review.presentation import write_review_html
from er_commons.human_review_support.extraction_review.records import RecordValidator
from er_commons.human_review_support.extraction_review.rendering import (
    PageRenderer,
    PdfiumPageRenderer,
)
from er_commons.human_review_support.extraction_review.selection import (
    SelectionResult,
    build_selection,
)
from er_commons.human_review_support.extraction_review.toc_decisions import (
    decision_record,
    load_toc_decisions,
)
...
def build_final_extraction_review(
    data_root: Path,
    gate_a_path: Path,
    output_root: Path,
    *,
    ...
    review_run_id: str,
    schema_root: Path,
    toc_decisions_path: Path | None,
    renderer: PageRenderer | None = None,
) -> Path:
    """Publish the six-queue final reviewer without rehashing large files."""
    if review_run_id != FINAL_REVIEW_RUN_ID:
        raise ValueError("review identity differs from the selected historical final-pass profile")
    ...
    try:
        inputs = discover_final_inputs(
            data_root, task_root=task_root, scope_id=scope_id, source_pdf_root=source_pdf_root
        )
...
def _build_review_plan(
    ...
    recheck_items, finding_recheck = _finding_recheck_items(
        review_parent, inputs.candidates, policy_sha256
    )
    return _ReviewPlan(
...
def _finding_recheck_items(
    prior_review_root: Path, candidates: dict[str, Path], policy_sha256: str
) -> tuple[tuple[ReviewItem, ...], dict[str, Any]]:
    """Make fresh valid-page cards for the accepted Task 03I finding."""
    path = prior_review_root / "records/finding_register.json"
...
__all__ = ["FINAL_REVIEW_RUN_ID", "build_final_extraction_review"]
