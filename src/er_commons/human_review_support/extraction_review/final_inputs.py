...
from er_commons.human_review_support.extraction_review.models import JsonObject, SourceEvidence

SCOPE_ID = "scopev1-bd4b7ca85b299ae528376b1a6e88b9d0fdba02e4f7e8862c5fa91a28b719e893"


def discover_final_inputs(
    data_root: Path, *, task_root: Path, scope_id: str, source_pdf_root: Path
) -> DiscoveredInputs:
    """Build typed final-pass inputs from the sealed Task 03J handoff."""
    if scope_id != SCOPE_ID:
        raise ValueError("scope differs from selected historical final-pass profile")
    ...
    sources = tuple(
        _source_evidence(data_root, task_root, order, completions, metadata, source_pdf_root)
