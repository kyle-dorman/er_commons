"""Resolve and verify one hierarchy-inference run before mutable work."""
...
from er_commons.hierarchy_inference.candidate_identity import build_candidate_identity
...
from er_commons.hierarchy_inference.config import (
    ...
    load_hierarchy_inference_config,
)
...
def prepare_run(
    ...
    *,
    ...
    config, effective_config_sha256 = load_hierarchy_inference_config(config_path)
    config_sha256 = (
        load_hierarchy_inference_config(config_identity_path)[1]
        if config_identity_path is not None
        else effective_config_sha256
    )
    inputs = load_hierarchy_inference_inputs(data_root, config)
