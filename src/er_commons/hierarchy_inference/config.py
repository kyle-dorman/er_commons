"""Strict configuration for deterministic hierarchy inference."""
...
class HierarchySource(StrictConfigModel):
...
class HierarchyInferenceConfig(StrictConfigModel):
    ...
    def validate_frozen_scope(self) -> HierarchyInferenceConfig:
        ...
        for path in (
            ...
            if path.is_absolute() or ".." in path.parts:
                raise ValueError("hierarchy-inference paths must be contained relative paths")
...
def load_hierarchy_inference_config(
    path: Path,
) -> tuple[HierarchyInferenceConfig, str]:
    """Load reviewed JSON config and return its exact byte checksum."""
    raw = path.read_bytes()
    return HierarchyInferenceConfig.model_validate_json(raw), hashlib.sha256(raw).hexdigest()
