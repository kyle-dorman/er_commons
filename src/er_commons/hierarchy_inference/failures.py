...
class FailureDisposition:
    ...
    def __post_init__(self) -> None:
        if self.fatal_code not in FATAL_CODES:
            raise ValueError(f"unknown hierarchy-inference fatal code: {self.fatal_code}")
        if not self.detail:
...
def explicit_failure(stage: RunStage, fatal_code: str, detail: str) -> HierarchyInferenceFailure:
