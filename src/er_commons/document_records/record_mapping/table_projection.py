...
def _docling_table(document: dict[str, Any], pointer: str) -> dict[str, Any]:
    ...
    if len(parts) != 3 or parts[:2] != ["#", "tables"] or not parts[2].isdigit():
        raise MappingContractError(f"invalid Docling table pointer in region mapping: {pointer}")
    tables = document.get("tables")
    index = int(parts[2])
    if not isinstance(tables, list) or index >= len(tables):
        raise MappingContractError(f"unknown Docling table pointer in region mapping: {pointer}")
    table = tables[index]
    if not isinstance(table, dict):
        raise MappingContractError(f"Docling table pointer is not an object: {pointer}")
    return table
...
def project_canonical_table_bundle(
    ...
    if any(table.family_id not in projected_family_ids for table in projected_tables):
        raise MappingContractError("canonical table projection left a table without a family")
