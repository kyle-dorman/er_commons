...
from er_commons.document_records.record_mapping.assets import AssetCatalog, raw_link
from er_commons.document_records.record_mapping.constants import BLOCK_TYPE_BY_LABEL, SCHEMA_VERSION
...
from er_commons.document_records.record_mapping.traversal import TraversalEvent
    ...
    for sequence, event in enumerate(context.block_events, start=1):
        ...
        if not projection.regions:
            raise MappingContractError(
                f"text has no valid canonical region: pointer={event.pointer}"
            )
        records.append(
            {
...
def _figures_and_images(
    *,
    ...
    for sequence, pointer in enumerate(context.figure_pointers, start=1):
        ...
        rejected.extend(projection.rejected)
        if not projection.regions:
            raise MappingContractError(f"picture has no valid canonical region: pointer={pointer}")
...
def _event_id(context: RecordMappingContext, event: TraversalEvent) -> str:
