"""Build the deterministic, explicitly non-release record-mapping identity."""
...
from er_commons.document_records.record_mapping.publication import sha256_file
...
def build_candidate_identity(
    *,
    ...
    identity: JsonObject = {
        ...
        "source_release": _source_release_identity(config, inputs, producer),
        "materialization_scope": _materialization_scope(config, inputs, mapping_policy_path),
        "docling": _docling_identity(conversion, inputs.conversion_runtime),
        "table_pipeline": _table_pipeline_identity(producer),
        "canonical_contract": {
        ...
        "producer_runs": [
            {
                ...
                "artifact_inventory_path": inventory_path.as_posix(),
...
def _docling_identity(conversion: JsonObject, runtime: JsonObject) -> JsonObject:
    models = conversion["model_inventory"]
    packages = conversion["package_versions"]
    return {
...
def _table_pipeline_identity(producer: JsonObject) -> JsonObject:
