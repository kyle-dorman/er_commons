...
from er_commons.document_records.record_mapping.bundle import BundleView, Record, regions
...
def _validate_region(view: BundleView, region: Record) -> None:
    ...
    if not (0 <= left < right <= width and 0 <= lower < upper <= height):
        raise MappingContractError(f"out-of-bounds region on {region['page_id']}")
    ...
    if region["units"] == "pdf_points":
        ...
        if scale is None:
            raise MappingContractError("pixel region is missing render scale")
        expected_width = page["width_pdf_points"] * scale
        expected_height = page["height_pdf_points"] * scale
    if (width, height) != (expected_width, expected_height):
        raise MappingContractError(f"region page geometry mismatch on {region['page_id']}")
...
def section_hierarchy_is_consistent(view: BundleView) -> None:
    ...
    for section_id in parent_by_section:
        ...
        while parent_id is not None:
            if parent_id in seen:
                raise MappingContractError(f"section hierarchy cycle at {section_id}")
            seen.add(parent_id)
    ...
    for section in view.bundle["sections"]:
        ...
        if set(section["ordered_child_ids"]) != actual_children:
            raise MappingContractError(f"section children differ for {section_id}")
    ...
    for document in view.bundle["documents"]:
        ...
        if not document["document_scope_complete"]:
            if document_family_ids or any(table["table_family_id"] for table in document_tables):
                raise MappingContractError("partial document emitted finalized table families")
            continue

        for table in document_tables:
            table_id = table["id"]
            if member_counts[table_id] != 1:
                raise MappingContractError(f"table is not assigned exactly once: {table_id}")
            if table["table_family_id"] not in document_family_ids:
                raise MappingContractError(f"table has unknown family: {table_id}")
            if family_id_by_table[table_id] != table["table_family_id"]:
                raise MappingContractError(f"table family membership differs: {table_id}")
    ...
    for table in view.bundle["tables"]:
        ...
        for cell in table["cells"]:
            ...
            if not (
                ...
                raise MappingContractError(f"cell is outside declared shape in {table['id']}")
            positions = {
                (row, column)
            ...
            if covered & positions:
                raise MappingContractError(f"overlapping cell position in {table['id']}")
            covered |= positions
        expected = {(row, column) for row in range(row_count) for column in range(column_count)}
        if covered != expected:
            raise MappingContractError(
                f"cell count differs from declared shape coverage in {table['id']}"
            )
...
def table_stage_mappings_are_consistent(view: BundleView) -> None:
    ...
    for observation in view.bundle["table_stage_observations"]:
        ...
        if not table_ids and not reason:
            raise MappingContractError("zero-table mapping requires an unmapped reason")
        if table_ids and reason is not None:
            raise MappingContractError("mapped table observation cannot have an unmapped reason")
        if len(table_ids) != len(set(table_ids)):
            raise MappingContractError("table observation merged duplicate table IDs")
...
def canonical_text_is_explainable(view: BundleView) -> None:
    ...
    for block in view.bundle["blocks"]:
        ...
        if block["canonical_text"] != expected:
            raise MappingContractError(f"unexplained canonical text change in {block['id']}")
...
def page_content_lists_are_complete(view: BundleView) -> None:
    ...
    for page in view.bundle["pages"]:
        ...
        if set(page["ordered_content_ids"]) != actual_content_ids:
            raise MappingContractError(f"page content differs for {page_id}")
