...
def build_cross_producer_bridge(
    ...
    *,
    ...
    if len(keys) != len(set(keys)):
        raise StructureContractError("bridge source contains duplicate stable-item keys")
    if set(canonical_block_by_key) & set(disposition_by_key):
        raise StructureContractError("bridge item cannot be both mapped and replaced")
    ...
    for item in items:
        ...
        if disposition is not None and disposition not in PERMITTED_BRIDGE_DISPOSITIONS:
            raise StructureContractError(f"bridge disposition is not permitted: {key}")
