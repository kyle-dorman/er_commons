...
def build_page_label_observations(
    *,
    ...
    if page_count < 1:
        raise StructureContractError("page-label construction requires at least one page")
    ...
    for feature in item_features:
        ...
        if page < 1 or page > page_count:
            raise StructureContractError(f"visible page-label evidence is out of range: {page}")
...
def _evidence(values: tuple[str, ...], evidence_ref: JsonObject | None) -> JsonObject:
    ...
    if evidence_ref is None:
        raise StructureContractError("present page-label evidence requires a checksum reference")
    if len(distinct) == 1:
