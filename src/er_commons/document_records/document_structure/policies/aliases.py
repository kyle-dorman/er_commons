...
def validate_target_aliases(view: DocumentStructureBundleView) -> None:
...
def _validate_alias_document_scope(view: DocumentStructureBundleView, alias: JsonObject) -> None:
    ...
    if alias["document_id"] != view.bundle["document_id"] or not view.belongs_to_document(
        ...
        raise StructureContractError(f"target alias escaped document scope: {alias['id']}")


def _validate_alias_order(view: DocumentStructureBundleView) -> None:
    ...
    if actual != expected:
        raise StructureContractError("target aliases are not in deterministic sequence")
    if aliases != sorted(aliases, key=lambda alias: _alias_sort_key(view, alias)):
        raise StructureContractError("target aliases are not in deterministic order")


def _alias_sort_key(view: DocumentStructureBundleView, alias: JsonObject) -> tuple[object, ...]:
    ...
    if target_orders != sorted(target_orders):
        raise StructureContractError(f"alias targets are not in document order: {alias['id']}")
    return (
...
def _target_document_order(view: DocumentStructureBundleView, target: JsonObject) -> int:
...
def _validate_unique_alias_key(
    ...
    if alias_key in seen_keys:
        raise StructureContractError(f"normalized alias key is duplicated: {alias_key}")
...
def _validate_collision_state(alias: JsonObject) -> None:
    ...
    if alias["resolution_status"] != expected_status or has_duplicate_target:
        raise StructureContractError(f"alias collision state differs for {alias['id']}")


def _validate_alias_targets(view: DocumentStructureBundleView, alias: JsonObject) -> None:
...
def _page_observation_by_id(view: DocumentStructureBundleView) -> dict[str, JsonObject]:
    ...
    if record_type != "document":
        raise StructureContractError(f"invalid canonical document ID: {view.bundle['document_id']}")
    return {
