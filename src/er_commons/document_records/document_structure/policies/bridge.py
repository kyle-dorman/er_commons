...
from er_commons.document_records.document_structure.constants import PERMITTED_BRIDGE_DISPOSITIONS
...
def _validate_unique_stable_keys(entries: list[JsonObject]) -> None:
    ...
    if len(stable_keys) != len(set(stable_keys)):
        raise StructureContractError("bridge stable-item keys collide")
...
def _validate_exact_source_evidence(
    ...
    for entry in entries:
        ...
        if actual != required:
            raise StructureContractError(f"bridge producer correspondence differs: {stable_key}")


def _validate_producer_ids(view: DocumentStructureBundleView, entry: JsonObject) -> None:
...
def _validate_mapped_entry(view: DocumentStructureBundleView, entry: JsonObject) -> str:
...
def _validate_unique_raw_pointers(entries: list[JsonObject]) -> None:
    ...
    for field in ("hierarchy_raw_pointer", "baseline_raw_pointer"):
        ...
        if len(pointers) != len(set(pointers)):
            raise StructureContractError(f"bridge {field} values collide")
