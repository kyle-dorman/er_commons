...
from er_commons.document_records.document_structure.constants import SEMANTIC_COUNT_FIELDS
...
def validate_control_provenance(control: dict[str, Any]) -> None:
    ...
    if set(control) != required:
        raise StructureContractError("bounded hierarchy control field set differs")
    if not isinstance(control["candidate_id"], str) or not _CANDIDATE_ID.fullmatch(
        control["candidate_id"]
    ):
        raise StructureContractError("bounded hierarchy candidate ID is invalid")
    for field in (
        "artifact_inventory_sha256",
        "semantic_file_set_sha256",
        "aggregate_semantic_sha256",
        ...
        if not isinstance(value, str) or not _SHA256.fullmatch(value):
            raise StructureContractError(f"bounded hierarchy {field} is invalid")
    if control["completion_status"] not in {"complete", "complete_with_ambiguities"}:
        raise StructureContractError("bounded hierarchy completion status is invalid")
    if control["acceptance_status"] != "accepted_with_known_limitations":
        raise StructureContractError("bounded hierarchy acceptance status differs")
    if control["corpus_wide_acceptance"] is not False:
        raise StructureContractError("bounded document control cannot claim corpus acceptance")
    if not isinstance(control["physical_page_count"], int) or control["physical_page_count"] < 1:
        raise StructureContractError("bounded hierarchy page count is invalid")
    ...
    if not isinstance(counts, dict) or set(counts) != SEMANTIC_COUNT_FIELDS:
        raise StructureContractError("bounded hierarchy semantic count fields differ")
    if not all(isinstance(value, int) and value >= 0 for value in counts.values()):
        raise StructureContractError("bounded hierarchy semantic counts are invalid")
...
def _validate_strict_control(control: dict[str, Any]) -> None:
    ...
    if set(control) != required:
        raise StructureContractError("strict hierarchy control field set differs")
    if control["corpus_wide_acceptance"] is not False:
        raise StructureContractError("strict document control cannot claim corpus acceptance")
