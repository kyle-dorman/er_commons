"""Build and serialize deterministic Task 03E.2 candidate records."""
...
from er_commons.hierarchy_correction.decisions import decisions_cover_features_in_order
...
def build_attempt_record(*, candidate_id: str, fatal_code: str, detail: str) -> JsonRecord:
    ...
    if fatal_code not in FATAL_CODES:
        raise ValueError(f"unknown hierarchy-correction fatal code: {fatal_code}")
    if not detail:
...
def prepare_candidate(
    *,
    ...
    decisions_cover_features_in_order(CorrectionBundleView(records))
