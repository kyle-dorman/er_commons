...
from er_commons.hierarchy_correction.semantic_types import LayoutState, NumberingKind
...
def align_parsed_line(text: str, parsed_page: dict[str, object]) -> LayoutEvidence:
    ...
    if not isinstance(cells, list):
        raise HierarchyCorrectionContractError("parsed page textline_cells is invalid")
    matches: list[str] = []
    for cell in cells:
        if not isinstance(cell, dict) or not isinstance(cell.get("text"), str):
            raise HierarchyCorrectionContractError("parsed page text line is invalid")
