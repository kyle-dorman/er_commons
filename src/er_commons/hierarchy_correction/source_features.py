...
def traverse_provenance_text(document: JsonObject) -> tuple[TraversedText, ...]:
    ...
    def visit(pointer: str) -> None:
        if pointer in active:
            raise HierarchyCorrectionContractError(f"Docling reading-order cycle: {pointer}")
    ...
    for root_name in ("body", "furniture"):
        ...
        if not isinstance(root, dict):
            raise HierarchyCorrectionContractError(f"Docling {root_name} root is invalid")
        for pointer in _references(root.get("children", []), f"{root_name} children"):
            visit(pointer)

    for index, raw in enumerate(text_items):
        if not isinstance(raw, dict):
            raise HierarchyCorrectionContractError(f"invalid Docling text: #/texts/{index}")
    ...
    *,
    ...
    for order, entry in enumerate(traverse_provenance_text(document)):
        ...
        if not isinstance(provenance, dict):
            raise HierarchyCorrectionContractError(f"invalid provenance: {entry.pointer}")
        ...
        if not isinstance(physical_page, int) or physical_page not in pages:
            raise HierarchyCorrectionContractError(f"missing conversion page: {entry.pointer}")
        if not isinstance(bbox, dict) or bbox.get("coord_origin") != "BOTTOMLEFT":
            raise HierarchyCorrectionContractError(f"invalid source bbox: {entry.pointer}")
        if not isinstance(charspan, list) or len(charspan) != 2:
            raise HierarchyCorrectionContractError(f"invalid source charspan: {entry.pointer}")
        text = item.get("text")
        orig = item.get("orig")
        role = item.get("label")
        if not isinstance(text, str) or not isinstance(orig, str) or not isinstance(role, str):
            raise HierarchyCorrectionContractError(f"invalid text fields: {entry.pointer}")
        page_record = pages[physical_page]
        parsed_page = page_record.get("parsed_page")
        if not isinstance(parsed_page, dict):
            raise HierarchyCorrectionContractError(f"missing parsed page: {physical_page}")
        ...
        if not isinstance(size, dict):
            raise HierarchyCorrectionContractError(f"missing page dimensions: {physical_page}")
        features.append(
            cast(
                ObservedItem,
                {
...
def build_feature_seeds(
    ...
    *,
    ...
    """Return schema-shaped observations for the legacy public facade."""
    return cast(
        list[JsonObject],
...
def _validated_picture_captions(
    ...
    if not isinstance(pictures, list):
        raise HierarchyCorrectionContractError("Docling pictures collection is invalid")
    for index, picture in enumerate(pictures):
        if not isinstance(picture, dict):
            raise HierarchyCorrectionContractError(f"invalid Docling picture: #/pictures/{index}")
        ...
        for caption in captions:
            ...
            if item is None or _pointer_collection(caption) != "texts":
                raise HierarchyCorrectionContractError(f"invalid picture caption ref: {caption}")
            if item.get("label") != "caption":
                raise HierarchyCorrectionContractError(f"picture caption label differs: {caption}")
            if _reference(item.get("parent"), f"caption parent: {caption}") != pointer:
                raise HierarchyCorrectionContractError(f"picture caption parent differs: {caption}")
            if _first_page(item, caption) != picture_page:
                raise HierarchyCorrectionContractError(f"picture caption page differs: {caption}")
...
def _index_objects(document: JsonObject) -> dict[str, JsonObject]:
    ...
    for collection in ("groups", "tables", "pictures", "texts"):
        ...
        if not isinstance(values, list):
            raise HierarchyCorrectionContractError(f"Docling {collection} collection is invalid")
        for index, item in enumerate(values):
            if not isinstance(item, dict):
                raise HierarchyCorrectionContractError(
                    f"invalid Docling {collection} item: {index}"
                )
            pointer = item.get("self_ref")
            expected = f"#/{collection}/{index}"
            if pointer != expected or pointer in objects:
                raise HierarchyCorrectionContractError(
                    f"invalid Docling self reference: {expected}"
                )
...
def _conversion_page_index(conversion_pages: JsonObject) -> dict[int, JsonObject]:
    ...
    if not isinstance(raw_pages, list):
        raise HierarchyCorrectionContractError("conversion pages collection is invalid")
    pages: dict[int, JsonObject] = {}
    for item in raw_pages:
        if not isinstance(item, dict) or not isinstance(item.get("page_no"), int):
            raise HierarchyCorrectionContractError("conversion page record is invalid")
        page_no = item["page_no"]
        if page_no in pages:
            raise HierarchyCorrectionContractError(f"duplicate conversion page: {page_no}")
        pages[page_no] = item
    return pages
...
def _references(value: Any, context: str) -> tuple[str, ...]:
    if not isinstance(value, list):
        raise HierarchyCorrectionContractError(f"invalid Docling reference list: {context}")
...
def _reference(value: Any, context: str) -> str:
    if not isinstance(value, dict) or not isinstance(value.get("$ref"), str):
        raise HierarchyCorrectionContractError(f"invalid Docling reference: {context}")
...
def _pointer_collection(pointer: str) -> str:
    ...
    if len(parts) != 3 or parts[0] != "#" or not parts[2].isdigit():
        raise HierarchyCorrectionContractError(f"invalid Docling pointer: {pointer}")
    return parts[1]
...
def _content_layer(item: JsonObject, pointer: str) -> Literal["body", "furniture"]:
    ...
    if layer not in {"body", "furniture"}:
        raise HierarchyCorrectionContractError(f"unknown content layer: {pointer}")
...
def _first_page(item: JsonObject, pointer: str) -> int:
    ...
    if not isinstance(provenance, list) or not provenance or not isinstance(provenance[0], dict):
        raise HierarchyCorrectionContractError(f"missing provenance: {pointer}")
    page = provenance[0].get("page_no")
    if not isinstance(page, int):
        raise HierarchyCorrectionContractError(f"invalid provenance page: {pointer}")
    return page


def _number(value: Any) -> float:
    if not isinstance(value, int | float) or isinstance(value, bool):
        raise HierarchyCorrectionContractError("bbox coordinate is invalid")
    return float(value)
