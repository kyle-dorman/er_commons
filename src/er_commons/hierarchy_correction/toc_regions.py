...
from er_commons.hierarchy_correction.text_evidence import normalize_text, parse_numbering
from er_commons.hierarchy_correction.toc_text import split_body_title
...
def detect_toc_regions(
    ...
    while index < len(features):
        ...
        if len(matches) > 1:
            raise HierarchyCorrectionContractError("visible TOC outline anchor is ambiguous")
...
def _outline_subtree_end(
    ...
    if parent_position is None:
        raise HierarchyCorrectionContractError("visible TOC outline parent is missing")
...
def _primary_region_end(
    ...
    if not later_pages:
        raise HierarchyCorrectionContractError("primary visible TOC has no later outline sibling")
...
def _embedded_region_end(
    ...
    raise HierarchyCorrectionContractError("TOC_REGION_UNTERMINATED")
...
def _first_body_index_on_or_after_page(features: list[JsonObject], page: int) -> int:
    for index, feature in enumerate(features):
        if feature["physical_page"] >= page and feature["content_layer"] == "body":
            return index
    raise HierarchyCorrectionContractError("visible TOC endpoint has no body item")
