"""Validate config joins and already-sealed downstream owner artifacts."""
...
if TYPE_CHECKING:
    from er_commons.corpus_extraction.lineage_preflight import ProducerLineage
...
def validate_lineage_bindings(
    *,
    ...
    canonical = _json(configs.canonical)
    ...
    if disposition.authority == "bounded_acceptance":
        authorization = _validate_bounded_binding(
            data_root, configs, source_id, disposition, correction, semantic, mismatches
        )
    if verify_seals:
        _validate_existing_candidates(
            data_root, root, semantic, cross_reference, disposition, mismatches
        )
    if mismatches:
        raise ValueError(
            "content-owner lineage preflight failed before PDF work: " + "; ".join(mismatches)
        )
    return final_root, authorization


def _validate_current_hierarchy_identity(
    data_root: Path,
    config_path: Path,
    semantic: JsonObject,
    mismatches: list[str],
) -> None:
    """Compare the configured downstream ID with the identity current code derives."""
    ...
    from er_commons.hierarchy_correction.preflight import prepare_run
    ...
    try:
        ...
        return
    configured_candidate_id = semantic.get("hierarchy_candidate_id")
...
def _producer_mismatches(
    ...
    expected = {
        "canonical baseline producer": (canonical.get("producer_run_id"), lineage.baseline),
        "correction hierarchy producer": (correction.get("producer_run_id"), lineage.hierarchy),
        "semantic baseline producer": (
            semantic.get("baseline_producer_run_id"),
            lineage.baseline,
        ),
        "semantic hierarchy producer": (
            semantic.get("hierarchy_producer_run_id"),
            lineage.hierarchy,
        ),
    }
    return [
...
def _validate_candidate_paths(semantic: JsonObject, mismatches: list[str]) -> None:
    pairs = (
        (
        ...
        (
            "semantic hierarchy candidate",
            "hierarchy_candidate_id",
            "hierarchy_candidate_relative_root",
        ),
    )
    for label, id_field, path_field in pairs:
        configured_id = semantic.get(id_field)
        configured_path = semantic.get(path_field)
...
def _validate_cross_reference_config(
    ...
    if not completion.is_file() or not inventory.is_file():
        mismatches.append(f"cross-reference semantic input is missing: {upstream_root}")
    else:
...
def _validate_bounded_binding(
    ...
    if relative is None:
        ...
        return None
    if semantic.get("bounded_acceptance_relative_path") != relative.as_posix():
        mismatches.append("semantic bounded-authorization path differs from run spec")
    ...
    if not isinstance(identity, dict) or not isinstance(scope, dict):
        ...
        return None
    if identity.get("candidate_id") != semantic.get("hierarchy_candidate_id"):
        mismatches.append("semantic hierarchy candidate differs from bounded authorization")
    if identity.get("config_sha256") != sha256_file(configs.hierarchy_correction):
...
def _validate_existing_candidates(
    ...
    baseline_id = semantic.get("baseline_candidate_id")
    baseline_path = semantic.get("baseline_candidate_relative_root")
    ...
    hierarchy_id = semantic.get("hierarchy_candidate_id")
    hierarchy_path = semantic.get("hierarchy_candidate_relative_root")
    ...
    if isinstance(upstream_id, str) and isinstance(artifact_root, str):
        _capture(
            "cross-reference semantic candidate",
...
def _verify_sealed_candidate(root: Path, candidate_id: str) -> None:
    """Verify common completion and inventory seals without importing an owner."""
