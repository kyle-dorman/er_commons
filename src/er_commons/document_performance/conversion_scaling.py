...
def write_conversion_scaling_ledger(
    data_root: Path, *, output_root: Path, run_relative_root: Path, source_id: str
) -> Path:
    """Write the exact metadata ledger without reading payload, PDF, or model bytes."""
    report = build_conversion_scaling_ledger(
        data_root, run_relative_root=run_relative_root, source_id=source_id
    )
    output = output_root / "task03h_gate1_scaling_ledger.json"
...
def write_conversion_pages_profile(
    ...
    *,
    ...
    parse_root = data_root.resolve() / run_relative_root / "document_parse_evidence"
    ...
    output = output_root / "task03h_conversion_pages_profile.json"
    write_json_atomic(output, report)
    return output


def bind_existing_conversion_pages_profile(
    data_root: Path, conversion_pages_path: Path, *, output_root: Path, run_relative_root: Path
) -> Path:
    ...
    report["input_seal"] = _sealed_payload_reference(
        data_root, conversion_pages_path.resolve(), run_relative_root=run_relative_root
    )
    write_json_atomic(output, report)
    return output
...
def write_document_level_overlay_profile(
    ...
    *,
    ...
    data_root = data_root.resolve()
    report = profile_document_level_overlay(baseline_path, heading_path, source_id=source_id)
    report["input_seals"] = [
        _sealed_payload_reference(data_root, path.resolve(), run_relative_root=run_relative_root)
        for path in (baseline_path, heading_path)
    ]
    output = output_root / "task03h_document_level_overlay_profile.json"
    write_json_atomic(output, report)
    return output


def write_alignment_scaling_benchmark(data_root: Path, *, output_root: Path) -> Path:
    """Write the bounded old-versus-indexed alignment scaling experiment."""
    report = benchmark_alignment_scaling()
    output = output_root / "task03h_alignment_scaling.json"
    write_json_atomic(output, report)
    return output
...
def write_table_bundle_comparison(
    ...
    *,
    ...
    data_root = data_root.resolve()
    report = compare_table_bundles(baseline_table_root, heading_table_root, source_id=source_id)
    report["input_seals"] = [
        _sealed_owner_reference(data_root, root.resolve().parents[3])
        for root in (baseline_table_root, heading_table_root)
    ]
    output = output_root / "task03h_table_bundle_comparison.json"
    write_json_atomic(output, report)
    return output
...
def write_assembled_reconstruction_profile(
    ...
    *,
    ...
    output = output_root / "task03h_assembled_reconstruction.json"
    write_json_atomic(output, report)
    return output
...
def profile_document_level_overlay(
    baseline_path: Path, heading_path: Path, *, source_id: str
) -> JsonObject:
...
def compare_table_bundles(baseline_root: Path, heading_root: Path, *, source_id: str) -> JsonObject:
...
def _sealed_payload_reference(
    data_root: Path, path: Path, *, run_relative_root: Path
) -> JsonObject:
    parse_root = data_root / run_relative_root / "document_parse_evidence"
...
def build_conversion_scaling_ledger(
    data_root: Path, *, run_relative_root: Path, source_id: str
) -> JsonObject:
    """Describe K2 payload copies and process timings from small seal records only."""
    data_root = data_root.resolve()
    task_root = data_root / run_relative_root
    parse_root = task_root / "document_parse_evidence"
    payloads = _payload_records(data_root, parse_root, source_id=source_id)
    return {
        "schema_version": "er_commons.task03h_gate1_scaling_ledger.v1",
        "source_id": source_id,
        "task_root": run_relative_root.as_posix(),
        "execution_boundary": {
            "source_pdf_bytes_read": False,
            "model_files_read": False,
            "docling_constructed": False,
        ...
        "raw_conversion_metrics": _raw_conversion_metrics(
            data_root, parse_root, source_id=source_id
        ),
        "derived_producer_metrics": _derived_producer_metrics(
            data_root, parse_root, source_id=source_id
        ),
        "document_process_timings": _process_timings(data_root, task_root, source_id=source_id),
...
def _payload_records(data_root: Path, parse_root: Path, *, source_id: str) -> list[JsonObject]:
...
def _process_timings(data_root: Path, task_root: Path, *, source_id: str) -> list[JsonObject]:
...
def _raw_conversion_metrics(
    data_root: Path, parse_root: Path, *, source_id: str
) -> list[JsonObject]:
