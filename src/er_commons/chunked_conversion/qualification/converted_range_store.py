...
from er_commons.chunked_conversion.gate_b import PageEvidence
from er_commons.chunked_conversion.gate_c import read_page_evidence, write_page_evidence
from er_commons.chunked_conversion.qualification.diagnostics import QualificationError
from er_commons.chunked_conversion.qualification.docling_adapter import RangeConversion
...
class ConvertedRangeStore:
    """Publish and verify the live page-evidence format used by Gate C workers."""
    ...
    def _verify_root(
        ...
        try:
            ...
            pages = read_page_evidence(root / "pages")
...
def retain_failure(root: Path, error: BaseException, *, stage: str) -> Path:
    ...
    write_json_atomic(
        failure_path,
        {
            "schema_version": "er_commons.task03h2_gate_c_failure.v2",
            "status": "failed",
            **diagnostic,
...
def _compact(page: PageEvidence) -> PageEvidence:
    from er_commons.chunked_conversion.gate_c import compact_page_evidence
...
def _dict_tuple(path: Path, key: str, *, stage: str) -> tuple[dict[str, Any], ...]:
    ...
    if not isinstance(value, list) or not all(isinstance(item, dict) for item in value):
        raise QualificationError("record_shape", stage=stage, path=path.as_posix())
