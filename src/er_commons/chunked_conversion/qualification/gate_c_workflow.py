"""Human-owned coordination for restartable Gate C qualification."""
...
from er_commons.chunked_conversion.qualification.converted_range_store import retain_failure
...
PublishCompletion = Callable[[Path, str, Path, RangePlan, ResourceObservation, int, bool], Path]


@dataclass(frozen=True)
class GateCWorkflowServices:
    """Inject execution and storage seams without weakening workflow policy."""

    verify_inputs: Callable[[Path, Path, Path], VerifiedG1Inputs]
    code_identity: Callable[[Path], G1CodeIdentity]
...
class GateCWorkflow:
    ...
    def __init__(
        ...
        *,
        project_root: Path,
        services: GateCWorkflowServices | None = None,
    ) -> None:
        self.project_root = project_root
        if services is None:
            from er_commons.chunked_conversion.qualification.gate_c_execution import (
    ...
    def run(self, request: GateCRequest) -> Path:
        """Run or reuse one identity-bound Gate C qualification."""
        ...
        plan = build_g1_plan(verified.sealed_identity, code)
        identity = gate_c_run_identity(
        ...
        run_id = gate_c_run_id(identity)
        run_root = request.output_root / "runs" / run_id
        completion_path = run_root / "records/completion_record.json"
        if completion_path.is_file():
            verify_gate_c_completion(
                run_root,
                ExpectedGateCCompletion(run_id=run_id, plan_id=plan.plan_id),
            )
            return completion_path
        self._prepare_run(run_root, run_id, plan, identity)
        store = self.services.range_store(run_root, plan)
        executed, reused = self._run_ranges(request, run_root, run_id, plan, store)
    ...
    def _prepare_run(
        ...
        (run_root / "ranges").mkdir(exist_ok=True)
        ...
        write_json_atomic(run_root / "records/boundary_evidence.json", boundary_evidence_record())
    ...
    def _run_ranges(
        ...
        for planned in plan.ranges:
            ...
            spec_path = run_root / "records" / f"worker_{planned.range_id}.json"
    ...
    def _write_interruption(
        ...
        write_json_atomic(
            path,
            {
                "schema_version": "er_commons.task03h2_gate_c_interruption.v1",
    ...
    def _write_range_report(
        ...
        write_json_atomic(
            ...
            {
                "schema_version": "er_commons.task03h2_gate_c_range_execution.v1",
                "run_id": run_id,
    ...
    def _run_aggregate(
        self, request: GateCRequest, run_root: Path, run_id: str, plan: RangePlan
    ) -> ResourceObservation:
...
__all__ = ["GateCWorkflow", "GateCWorkflowServices"]
