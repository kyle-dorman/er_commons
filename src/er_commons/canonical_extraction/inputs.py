...
from er_commons.source_freeze import SourceManifest, SourceRecord, SourceRole, assert_contained

JsonObject = dict[str, Any]


@dataclass(frozen=True)
class CanonicalizationInputs:
...
def _verify_selected_source(
    ...
    if record.source_role != SourceRole.MODEL_CORPUS:
        raise ValueError("selected canonicalization source is not model_corpus")
    expected = (
...
def load_canonicalization_inputs(
    data_root: Path,
    config: CanonicalizationConfig,
