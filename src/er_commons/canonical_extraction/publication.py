...
def verify_completed_candidate(root: Path, candidate_id: str) -> Path:
    ...
    if not completion_path.is_file() or not inventory_path.is_file():
        raise ContractError(f"candidate terminal records are missing below {root}")
    completion = json.loads(completion_path.read_text())
    inventory = json.loads(inventory_path.read_text())
    if completion.get("candidate_id") != candidate_id:
        raise ContractError("candidate completion identity differs")
    if completion.get("release_candidate") is not False:
        raise ContractError("Task 03D completion is not marked non-release")
    if completion.get("artifact_inventory_sha256") != sha256_file(inventory_path):
        raise ContractError("candidate completion does not seal its inventory")
    for item in inventory.get("files", []):
        relative = Path(item["path"])
        if relative.is_absolute() or ".." in relative.parts:
            raise ContractError(f"unsafe candidate inventory path: {relative}")
        path = root / relative
        if not path.is_file():
            raise ContractError(f"candidate inventory file is missing: {relative}")
        if path.stat().st_size != item["byte_size"] or sha256_file(path) != item["sha256"]:
            raise ContractError(f"candidate inventory checksum differs: {relative}")
    if build_inventory(root) != inventory:
        raise ContractError("candidate inventory differs from the managed file set")
