...
class DoclingTraversal:
    ...
    def run(self) -> TraversalResult:
        ...
        if not isinstance(body, dict):
            raise ContractError("saved Docling document has no body object")
    ...
    def _visit(self, pointer: str) -> None:
        ...
        if collection == "groups":
            ...
            raise ContractError(f"unsupported Docling pointer in body graph: {pointer}")

    def _visit_group(self, pointer: str) -> None:
        if pointer in self._active_groups:
            raise ContractError(f"Docling group cycle: {pointer}")
    ...
    def _suppress_descendant_text(self, pointer: str, exceptions: set[str]) -> None:
        ...
        if collection == "texts":
            if pointer in self._emitted_text:
                raise ContractError(f"cannot suppress already emitted text: {pointer}")
    ...
    def _emit_text(self, pointer: str, layer: str) -> None:
        ...
        if pointer in self._suppressed_text:
            raise ContractError(f"suppressed Docling text was emitted: {pointer}")
        if layer not in {"body", "furniture"}:
            raise ContractError(f"unsupported content layer on {pointer}: {layer}")
    ...
    def _resolve(self, pointer: str) -> dict[str, Any]:
        ...
        if not isinstance(values, list) or index >= len(values):
            raise ContractError(f"unknown Docling pointer: {pointer}")
        item = values[index]
        if not isinstance(item, dict):
            raise ContractError(f"Docling pointer does not resolve to an object: {pointer}")
        return item

    @staticmethod
    def _child_pointer(reference: Any) -> str:
        if not isinstance(reference, dict) or not isinstance(reference.get("$ref"), str):
            raise ContractError("Docling child is missing a JSON pointer")
    ...
    def _parse_pointer(pointer: str) -> tuple[str, int]:
        ...
        if len(parts) != 3 or parts[0] != "#" or not parts[2].isdigit():
            raise ContractError(f"unsupported Docling JSON pointer: {pointer}")
    ...
    def _content_layer(item: dict[str, Any]) -> Literal["body", "furniture"]:
        layer = item.get("content_layer", "body")
        if layer not in {"body", "furniture"}:
            raise ContractError(f"unsupported Docling content layer: {layer}")
