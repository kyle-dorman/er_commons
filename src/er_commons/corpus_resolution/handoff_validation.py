"""Read-only validation of a published Task 03 corpus handoff."""
...
from er_commons.corpus_extraction.candidates import verify_identity_and_upstreams
...
def validate_handoff(
    *,
    ...
    """Verify the v1.1 bundle and every successful document candidate in place."""
    ...
    validate_contract_bundle(bundle, DirectoryArtifactReader(root))
...
def _verify_document_candidate(
    ...
    if (
        ...
        raise ValueError("document identity differs from corpus handoff")
...
def _infer_data_root(extraction_root: Path, bundle: JsonObject) -> Path:
    """Find the unique ancestor that resolves every sealed owner completion."""
