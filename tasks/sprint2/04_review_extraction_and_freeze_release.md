# Task 04: Pilot Extraction Review and Freeze the Accepted Release

Status: **prepared; not activated**. The first pass waits for explicit user
activation. It uses retained Task 03H evidence only as a diagnostic review
dataset, pauses for [Task 03I](03i_remediate_task04_review_findings.md), and
resumes for the final release decision only after [Task
03J](03j_run_final_canonical_extraction.md) publishes a fresh machine handoff.
...
The first pass tests the review workflow and produces an evidence-anchored
finding list. It does not accept the Task 03H corpus. Accepted extraction
defects pass to Task 03I; review-tool defects remain in Task 04. Task 03J then
runs all 35 sources under a fresh namespace and identity. Task 04 finally
regenerates the review evidence against that handoff, verifies remediation and
fresh risk samples, records usability separately from machine artifacts, and
either freezes the accepted release or stops on a material defect.
...
3. After Task 03I and Task 03J, make the independent final usability decision
   and freeze the accepted extraction release and its separate review registry.
...
- the closed Task 03I finding disposition and fresh Task 03J handoff, but only
  for the final Task 04 pass.
...
### Final pass against Task 03J

- a newly generated selection and review bundle bound to Task 03J identities
  and checksums rather than mechanically reusing Task 03H anchors;
- explicit rechecks of every repaired finding plus fresh stratified and
  risk-triggered samples;
- a separate usability registry keyed by Task 03J corpus and source identity,
  adding canonical page, table, or family IDs only when they exist;
- an all-source review summary, unresolved-risk report, and release-freeze
  record; and
- a clear stop outcome instead of a freeze when a material defect remains.
...
Task 03H diagnostic evidence
  ...
  -> Task 04 final review, usability registry, and release freeze
```

- Task 04 owns human inspection, review-tool quality, usability observations,
  the finding register, and the final release decision.
...
Task 04 pauses after its user-approved first-pass finding register. Task 03I
and Task 03J close independently before Task 04 resumes.
...
5. Pause Task 04 without accepting or freezing Task 03H.

### Gate D: resume after Task 03I and Task 03J

1. Verify the closed Task 03I disposition and fresh Task 03J machine handoff.
2. Regenerate or re-resolve every review selection against Task 03J identities;
   reject stale IDs, checksums, pages, or render inputs.
3. Recheck all repaired findings and perform fresh stratified/risk review using
   the same declared policy.
4. Validate all-source terminal accounting and machine integrity.
5. Publish the separate usability registry and freeze record, or stop and route
   a material extraction finding back to a separately bounded Task 03 stage.
...
- **Task ownership:** Are tool defects confined to Task 04, extraction defects
  routed only through Task 03I, full execution confined to Task 03J, and human
  dispositions absent from machine candidates?
...
- Task 03J's all-source handoff passes its owning machine validators and no Task
  03H production completion contributed to it;
- every first-pass extraction finding has a closed Task 03I disposition and an
  exact Task 03J recheck where applicable;
- all final review items and renders bind to Task 03J identities and checksums;
- each source has an explicit human usability disposition, with page/table/
  family decisions added only where reviewed evidence supports them;
- reviewed tables are explicitly verified or rejected, all other tables remain
  unverified and ineligible, and downstream admission of an unreviewed table is
  blocked until a separately versioned render-backed review extends the
  registry;
- the usability registry remains separate from Task 03 machine records; and
- the freeze record pins the exact Task 03J candidate, registry version, review
  selection, generated-render manifest, findings, and unresolved limitations.

If the main report is materially unusable, or another source remains materially
unusable while the user requires every document to be retained, stop for an
explicit remediation or benchmark-policy decision rather than silently exclude
the source or prefill a usable disposition.
